...
% total_D_position=importdata('D_position.txt');
% %% 乘客出发地及目的地
% total_P_position=importdata('P_position.txt');
% %% 出租车可用座位数量
% total_D_seat_number=importdata('D_seat_number.txt');
% %% 乘客预定座位数量
% total_P_seat_number=importdata('P_seat_number.txt');
% %% 乘客期望上车时间
% total_P_WT=importdata('P_TW.txt');
% %% 分区数量
% Num=3;
% 
% 
...
h1.XData = {'Travel cost','Contact','Waiting time'};
clim([0,1]);
ax=gca;
...
h.XData = {'Travel cost','Contact','Waiting time'};
clim([0,1]);
ax=gca;
ax.YDisplayLabels = nan(size(ax.YDisplayData));
%title('优化后的种群')
...
% figure
% scatter3(functionvalue0(:,1),functionvalue0(:,2),functionvalue0(:,3));
% hold on
