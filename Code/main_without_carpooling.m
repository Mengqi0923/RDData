...
w2=0.5;                           %延迟时间权重
%% 初始化种群
Chrom=init_pop_No_Carpool(NIND,codelength,n/2,P_WT,P_seat_number,D_seat_number);

%% 计算初始种群的目标函数值
[cost,wt] = FounctionValueNoCarpool(Chrom,n/2,m,P_WT,distanceD2P,distanceP2P,w1,w2,v);
...
functionvalue=zeros(size(Chrom,1),2);           %合并后种群的各目标函数值
...
%     newChrom = Mutate( newChrom,n/2,m,Pm,P_seat_number,D_seat_number);
    ...
    [newcost,newwt] = FounctionValueNoCarpool(newChrom,n/2,m,P_WT,distanceD2P,distanceP2P,w1,w2,v);
    %% 计算新种群目标函数值
    ...
    functionvalue=zeros(size(newChrom,1),2)
    functionvalue(:,1)=cost
    functionvalue(:,2)=wt;
